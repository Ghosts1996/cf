...
# Сколько секунд максимум ждать ответ одной панели 3x-ui при параллельном
# опросе ключей пользователя (см. api_user_keys), прежде чем оставить
# connection_string этого ключа пустым и отдать ответ дальше.
...
def api_user_keys():
    ...
    # Опрашиваем панели параллельно через пул потоков (сеть — I/O-bound,
    # GIL здесь не мешает) и ограничиваем ожидание каждого результата.
    # Последовательный опрос без таймаута растягивал ответ /user/keys на
    # время самой медленной панели, умноженное на число ключей: у владельца
    # GLOBAL-бандла ключи лежат сразу на всех локациях, и одна недоступная
    # панель превращалась в долгий спиннер во всём приложении — этот
    # эндпоинт дёргают пять экранов сразу. При ошибке или таймауте
    # конкретного ключа его connection_string остаётся пустым, весь запрос
    # из-за этого не падает.
    if keys:
...
def api_hosts():
    hosts = get_all_hosts()
    # host_url — приватный адрес админ-панели 3x-ui — клиенту не отдаём.
    # Эндпоинт защищён только статическим @require_api_key, а этот ключ
    # достаётся из любого собранного APK реверс-инжинирингом.
    cleaned_hosts = []
    for h in hosts:
        # connect_host/connect_port — публичный домен из subscription_url,
        # который и так уходит пользователю в его VLESS-подписке. Нужен
        # приложению, чтобы измерить живой пинг с телефона до сервера
        # (app/lib/screens/servers_screen.dart). Это не host_url.
        connect_host = None
        connect_port = 443
