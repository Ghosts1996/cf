...
**XHTTP (он же splithttp) ядро не поддерживает** — это транспорт Xray-core,
в sing-box его нет ни в одной версии. Конфиг с таким блоком ядро отвергает
на `checkConfig` целиком, вместе со всеми остальными локациями подписки,
попавшими в тот же конфиг. Поэтому `_isTransportSupported()` отсеивает такие
профили до сборки конфига: они не попадают ни в группу-селектор, ни в
перебор при подключении, а на экране «Мои ключи» кнопка «Проверить ключ»
показывает их отдельным списком с причиной. Чтобы такие локации заработали,
нужен либо клиент на Xray-core (другое ядро, не этот пакет), либо перевод
инбаунда на панели на `httpupgrade` или `ws` — их sing-box понимает.
...
- **XHTTP (splithttp) не поддерживается этим ядром.** В апстримном sing-box
  транспорта нет — он есть в форке `hiddify-sing-box`, на котором работает
  Hiddify. Локации с `type=xhttp` отсеиваются до сборки конфига (иначе ядро
  отвергло бы конфиг целиком, вместе с рабочими локациями), а «Проверить
  ключ» показывает их отдельным списком. Варианты: перевести инбаунд на
  панели на `httpupgrade` или `ws`, либо взять ядро с поддержкой — готовая
  реализация лежит в ветке `claude/xhttp-support` (там же инструкция
  `app/XHTTP_CORE.md` и workflow сборки ядра).
- **Legacy-синтаксис sing-box в двух местах** — блокирующий конфиг строгого
  Kill Switch и правило обхода LAN (см. Troubleshooting). Ядро 1.12.x их
  принимает, в 1.13/1.14 они удалены: перед обновлением ядра нужно
  перевести их на `action`-синтаксис.
...
**Исправлено** в основном конфиге — там теперь `route.rules` с полем
`action` (`sniff`, `hijack-dns`, `reject`). Остались два места со старым
синтаксисом, которые ядро 1.12.x ещё принимает: блокирующий конфиг
строгого Kill Switch (`_buildBlockAllConfig` — outbound'ы `block` и `dns`)
и правило обхода LAN (`{'ip_is_private': true, 'outbound': 'direct'}`).
Их нужно перевести на `action`-синтаксис до обновления ядра на 1.13+.
