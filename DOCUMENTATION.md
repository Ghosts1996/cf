...
- **XHTTP (splithttp) не поддерживается ядром.** Это транспорт Xray-core,
  в sing-box его нет. Локации подписки с `type=xhttp` отсеиваются до сборки
  конфига (иначе ядро отвергло бы конфиг целиком, вместе с рабочими
  локациями), а «Проверить ключ» на экране «Мои ключи» показывает их
  отдельным списком. Варианты: перевести инбаунд на панели на `httpupgrade`
  или `ws`, либо использовать клиент на Xray-core.
