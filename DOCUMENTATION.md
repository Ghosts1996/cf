...
├── SECURITY.md                    Аудит уязвимостей приложения и патча бэкенда
...
`TunnelService._buildSingBoxConfig()` собирает JSON-конфиг под ядро, которое
лежит в пакете `flutter_singbox_client` (`android/libs/libbox.aar`). Ядро
текущей сборки — **sing-box 1.12.x**: оно ещё принимает `block`/`dns`
outbound'ы и `outbound` в правилах маршрутизации, но уже предупреждает, что
их удалят в 1.13/1.14 (см. «Известные ограничения»).

Структура конфига:

- **Outbound'ы локаций** — `type: vless`, TLS + `utls` (fingerprint chrome по
  умолчанию), при `security=reality` — блок `reality` с `public_key` и
  `short_id`; `multiplex` (Mux) добавляется, только если у профиля нет `flow`.
- **Группа-`selector` с тегом `proxy`** — когда в подписке больше одной
  локации, все они кладутся в конфиг как `out-0`, `out-1`, … и собираются в
  селектор. Это позволяет менять страну без разрыва туннеля
  (`switchPreferredHost` → `selectOutbound`) и мерить задержку по всем
  локациям силами самого ядра (`urlTest`). При одной локации конфиг прежний:
  один outbound с тегом `proxy`.
- **Outbound `direct`** — прямой выход для LAN-правила и `route.final`.
- **DNS** — DoT к фиксированному IP выбранного провайдера (`type: tls`, порт
  853, корректный `server_name` для сертификата); для пользовательского
  адреса — DoH (`type: https`). У сервера всегда `detour: proxy`: DNS
  перехватывается правилом `hijack-dns` и должен идти через туннель. При
  включённом Fake IP добавляется сервер `fakeip` и правило для запросов
  A/AAAA. `strategy` — `ipv4_only`, а при включённом IPv6 — `prefer_ipv4`.
  Отдельно объявлен системный резолвер `local-dns`, на который ссылается
  `route.default_domain_resolver`: им ядро разрешает адрес самого
  VLESS-сервера, если тот задан доменом. Без этого резолв уходил бы в
  `remote-dns` с `detour: proxy`, то есть через ещё не поднятый туннель.
- **Route rules** — по порядку: обход LAN (`ip_is_private`, если включён),
  `sniff`, блокировка рекламы по `domain_suffix` (если включена),
  `hijack-dns`, фрагментация TLS при включённом обходе DPI.
  `route.final = 'proxy'`.
- **Inbound'ы** — `mixed` на `127.0.0.1:2080` поднимается всегда (через него
  идут пробники задержки и проверка связности), а в VPN-режиме к нему
  добавляется `tun` (`172.19.0.1/28`, `auto_route`, `strict_route`,
  `stack: mixed`, `include_package`/`exclude_package` из
  split-туннелирования).

### Транспорты, которые понимает ядро

`tcp` (без блока `transport`), `ws`, `grpc`, `http`, `httpupgrade`, `quic`.

**XHTTP (он же splithttp)** есть только в форке `hiddify-sing-box`, на
котором работает Hiddify: в апстримном sing-box этого транспорта нет. В этой
ветке приложение собрано именно с таким ядром (`pubspec.yaml` ->
`ccurecc_singbox_xhttp`), в рабочей — с прежним. Приложение собирать такой конфиг умеет (формат —
как у Hiddify, см. `_buildXhttpTransport`), а поддержку проверяет у самого
ядра: отдаёт на `checkConfig` минимальный конфиг с XHTTP и запоминает ответ
(`_coreSupportsTransport`).

- ядро без XHTTP — профили отсеиваются до сборки конфига, чтобы не утащить
  за собой остальные локации, а «Проверить ключ» показывает их отдельным
  списком с причиной;
- ядро с XHTTP — профили используются наравне с остальными, включая
  группу-селектор и замер задержки.

Как ядро собирается и подключается — в `app/XHTTP_CORE.md`. Альтернатива без
смены ядра: перевести инбаунд на панели на `httpupgrade` или `ws`.

Логика `connect()`: подгружает все профили из подписки (или один — если
`connection_string` уже `vless://...`), пробует каждый по очереди
(`checkConfig` → `connect` → ожидание `connected` до 12 секунд → проверка
связности), при неудаче переходит к следующему; если ни один не подключился —
бросает `TunnelException` с числом опробованных серверов и последней
причиной.

Kill Switch: при обрыве не по инициативе пользователя и включённом тумблере —
до 3 попыток авто-переподключения (8 при «агрессивном» режиме) с интервалом
3 секунды. Если попытки исчерпаны и включён строгий режим, поднимается
служебная сессия с `route.final = block`, которая держит трафик
заблокированным до отключения вручную.
...
Полный разбор — в `SECURITY.md` (аудит приложения и патча бэкенда).
...
Не проверено и не в зоне ответственности кода: реальная схема таблиц форка
бота (см. `SECURITY.md`), сетевой rate-limiting (уровень nginx/Cloudflare,
не приложения), полноценный пентест боевого окружения.
...
- **XHTTP работает только на ядре из форка.** В этой ветке оно и стоит
  (`ccurecc_singbox_xhttp`, собрано из `hiddify/hiddify-sing-box @ 8d94f44`
  под arm64 и arm). Приложение определяет поддержку у ядра само, так что тот
  же код на прежнем ядре просто отсеивает XHTTP-локации.
- **Ядро из форка собрано без tailscale и naive-outbound.** Первый на ветке
  `extended` не собирается — подмодуль отстал от кода ядра; второй тянет
  cronet на сотни мегабайт. Ни то, ни другое приложению не нужно. Из-за
  этого же в ядре нет архитектур x86: эмуляторы на нём не заработают.
- **Совместимость конфига с ядром проверяется тестом.** `app/test/`
  выгружает конфиги во всех сочетаниях настроек, и их можно прогнать через
  `sing-box check` любой сборки ядра. Сейчас все 140 конфигов (138 обычных
  плюс два с XHTTP) принимает форк Hiddify, а 138 обычных — и апстрим
  1.12.9, оба без предупреждений.
- **Тумблер «Защита от DNS-протечек» не меняет поведение.** DNS всегда идёт
  через туннель (`detour: proxy` у DNS-сервера), потому что его перехватывает
  правило `hijack-dns`. Выключение тумблера сейчас ни на что не влияет.
...
дополнительных опций — sing-box считает такой `detour` бессмысленным и
падает при старте. **Исправлено**: у DNS-сервера теперь стоит
`detour: 'proxy'` — DNS перехватывается правилом `hijack-dns` и в любом
случае должен идти через туннель, так что на `direct` он больше не
ссылается.

### `INVALID_CONFIG` / `legacy inbound fields are deprecated in sing-box 1.11.0 and removed in sing-box 1.13.0`

Причина: конфиг использовал синтаксис, который ядро уже не принимает:
...
**Исправлено** — везде, включая блокирующий конфиг строгого Kill Switch и
правило обхода LAN, используется `action`-синтаксис (`sniff`, `hijack-dns`,
`reject`, `route`). Проверено на собранных из исходников ядрах 1.12.9 и
форке на 1.13: конфиги принимаются без предупреждений.
...
`SHOPBOT_API_KEY` обязателен: значения по умолчанию в `main.dart` нет, ключ
передаётся через `--dart-define` или GitHub Secret. Иначе он был бы виден
любому, кто открывает `main.dart` в репозитории. Актуальное состояние — в
разделе [«Секреты и конфигурация»](#секреты-и-конфигурация).
