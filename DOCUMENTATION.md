...
**XHTTP (он же splithttp)** штатное ядро не поддерживает: в апстримном
sing-box этого транспорта нет, он есть только в форке `hiddify-sing-box`, на
котором работает Hiddify. Приложение собирать такой конфиг умеет (формат —
...
Как получить и подставить такое ядро, не трогая рабочую сборку, — в
`app/XHTTP_CORE.md`. Альтернатива без смены ядра: перевести инбаунд на
панели на `httpupgrade` или `ws`.
...
- **XHTTP (splithttp) требует другого ядра.** В апстримном sing-box этого
  транспорта нет, он есть в форке `hiddify-sing-box`. Код приложения к нему
  готов и определяет поддержку у ядра сам; как собрать и подставить такое
  ядро — в `app/XHTTP_CORE.md`. Пока ядро прежнее, такие локации
  отсеиваются, а «Проверить ключ» показывает их отдельным списком.
- **Совместимость конфига с ядром проверяется тестом.** `app/test/`
  ...
  `sing-box check` любой сборки ядра. Сейчас все 138 конфигов принимают и
  апстрим 1.12.9, и форк Hiddify — без предупреждений.
