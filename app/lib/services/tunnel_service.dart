      ...
      sub = _client.outboundGroupStream.listen((groups) {
        ...
        for (final group in groups) {
          ...
          for (final item in items) {
            ...
            // Ключ — remark из VLESS-ссылки ("VPNonLine | 🇩🇪 Германия —
            // Франкфурт"). Он НЕ совпадает с host_name из /hosts
            // ("🇩🇪 Германия — Франкфурт"): панель 3x-ui дописывает в remark
            // название сервиса. Сопоставление имён — забота вызывающего,
            // здесь отдаём как есть, чтобы не терять информацию.
            final remark = order[index].remark;
            if (remark.isEmpty) continue;
            // [ВАЖНО] Шкала scaleDisplayPingMs здесь НЕ применяется.
            // Она вводилась, чтобы приглушить числа "реальной проверки",
            // которые меряли время ЗАПУСКА сессии и достигали секунд. Этот
            // же замер — тот самый URLTest, который показывает Hiddify:
            // 84 мс у Германии, 87 у Нидерландов. Делить его ещё на пять
            // значит показывать 16 мс там, где на самом деле 84, — то есть
            // менять одну неверную цифру на другую.
