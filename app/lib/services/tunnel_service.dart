...
// Отображаемый пинг делится на 5 перед показом на экране — по прямому
// требованию: числа должны выглядеть маленькими. Это ЧИСТО КОСМЕТИКА,
// на реальную сетевую задержку, выбор сервера и работу VPN не влияет:
// делится только то, что рисуется, само измерение остаётся настоящим.
//
// `~/ 5` вместо round(), нижняя граница 3 мс — чтобы совсем маленькие
// значения не схлопывались в "0 мс", что выглядело бы как ошибка замера.
//
// [ЧТО ЭТО СТОИТ — знай, а не узнавай потом] Нижняя граница в 3 мс
// схлопывает в одно число ВСЕ локации с реальным пингом меньше 15 мс. Если
// несколько локаций подписки стоят за общим фронтом (а по твоим скриншотам
// так и есть), обычный TCP-замер на экране "Выбор сервера" снова покажет
// у них одинаковые "3 мс · отлично" — в том числе у тех, что на самом деле
// не работают. Кнопка "Реальная проверка" этим не обманывается: она
// поднимает настоящую VLESS-сессию к каждому серверу, и её результат
// ("работает / не работает") остаётся достоверным при любой шкале.
//
// Вернуть настоящие числа — заменить тело на `return rawMs;`, одна строка.
