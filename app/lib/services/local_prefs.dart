...
class PrefKeys {
  ...
  // [НОВОЕ] JSON с тем, ЧЕМ поднят текущий туннель: connection_string
  // подписки, предпочтённая локация и имя сервера, к которому реально
  // подключились. Пишется в TunnelService только после успешного
  // подключения и стирается в disconnect() — см. докстринг
  // TunnelService._persistSessionRoute(). Нужен ровно для одного: после
  // перезапуска процесса приложения (свайп из списка задач при работающем
  // в фоне VpnService) вернуть эти три значения, которые до этого жили
  // только в памяти. Без них на живом туннеле не работали ни кнопка
  // "Сменить сервер" ("Нет активного туннеля, который можно переключить"),
  // ни авто-переподключение Kill Switch.
  //
  // [ВНИМАНИЕ — прочитай] Здесь на диск в ОТКРЫТОМ виде ложится ссылка на
  // подписку (или сама vless://-ссылка) — тот, у кого есть доступ к
  // файлам приложения, получит доступ к VPN. Это не новый класс риска:
  // ручной ключ (PrefKeys.manualConnectionString выше) хранится там же и
  // так же с самого начала. Но раз данных такого рода в SharedPreferences
  // стало больше — в AndroidManifest.xml стоит явно выставить
  // android:allowBackup="false" (сейчас не задано, а значение по умолчанию
  // — true, то есть содержимое SharedPreferences может уехать в облачный
  // бэкап/adb backup). Я это НЕ менял сам: отключение бэкапа означает, что
  // пользователи потеряют настройки при переносе на новый телефон, это
  ...
  // [ИСПРАВЛЕНО] Обход локальной сети (LAN). По умолчанию ВЫКЛЮЧЕН —
  // пользователь включает сам. Когда включено: устройства в локальной
  // сети (роутер, принтер, NAS, IoT) видны напрямую, без туннеля. Когда
  // выключено (значение по умолчанию) — весь трафик, включая LAN, идёт
  // через VPN. См. route.rules -> ip_is_private в tunnel_service.dart.
  ...
  // [НОВОЕ — офлайн-кэш подписки] Реальный баг с реального скриншота: при
  // перезаходе в приложение (или на слабой мобильной сети сразу после
  // холодного старта) `TunnelService._loadProfiles()` качает подписку
  // заново по сети — раньше распарсенные VLESS-профили держались ТОЛЬКО в
  // памяти (`_cachedProfiles` в tunnel_service.dart), то есть каждый
  // перезапуск процесса стирал этот кэш подчистую. Если именно в
  // этот момент сеть ещё не готова (типичная ситуация в первые секунды
  // после старта — модем ещё не поднял канал, DNS ещё не отвечает) —
  // пользователь получает "Не удалось загрузить конфигурацию подписки",
  // хотя реальный VLESS-туннель из ПРЕДЫДУЩЕЙ сессии всё это время
  // продолжает штатно работать в фоне (отсюда и рассинхрон "в шторке VPN
  // работает, а в приложении ошибка"). Три ключа ниже сохраняют на диск
  // сырое тело последней УСПЕШНО скачанной подписки — при следующем
  // холодном старте или временном сбое сети `_loadProfiles()` может сразу
  // подставить их вместо того, чтобы ждать (или падать с ошибкой) заново.
  ...
  // [НОВОЕ — офлайн-кэш списка ключей] Та же идея, что и выше, но для
  // `GET /user/keys` (см. connect_screen.dart::_loadKeyState). Позволяет
  // главному экрану мгновенно показать последний известный ключ ещё ДО
  // ответа сервера (а не спиннер/"НЕТ КЛЮЧА" все 2-3 попытки таймаута) и
  // не терять из виду свой ключ вовсе без интернета — тумблер "Подключить"
  // на кэшированном ключе всё ещё доступен, если реальная подписка на этот
  // ключ тоже лежит в кэше выше.
  // [НОВОЕ] Кэш последнего успешно полученного /user/profile (баланс,
  // email, реферальные поля) — чтобы экран "Баланс" после входа показывал
  // данные сразу и не превращался в прочерки при любом сбое сети.
  // [НОВОЕ] Пускать ли трафик САМОГО приложения (личный кабинет, баланс,
  // список ключей, замеры пинга) мимо VPN-туннеля.
  //
  // Зачем это вообще: пока туннель поднят, запросы приложения идут через
  // него же. Если туннель поднялся, но пакеты через него не ходят —
  // приложение слепнет вместе со всем телефоном: /user/keys не отвечает,
  // экран пишет "НЕТ КЛЮЧА" при двух активных ключах, все локации
  // показывают "недоступен", а выключить VPN можно только из шторки. Так
  // делают все зрелые VPN-клиенты: своё управляющее соединение они держат
  // вне туннеля.
  //
  // ПО УМОЛЧАНИЮ ВКЛЮЧЕНО (true). Если после обновления окажется, что VPN
  // стал вести себя хуже — поставь здесь fallback: false в
  // tunnel_service.dart (одна строка, помечена там же), и поведение
  // вернётся ровно к прежнему.
  //
