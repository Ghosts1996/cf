...
class WindowsSingboxRuntime implements SingboxRuntimeClient {
  ...
  Future<void> initialize() async {
    // [НОВОЕ — реальная причина "приложение сразу при открытии уже
    // Подключено", "не до конца удаляется при деинсталляции",
    // "иконка в трее не закрывается" — всё это разные симптомы одной
    // причины] У этого приложения на Windows НЕТ логики системного трея —
    // если после закрытия окна где-то видна его иконка, это не оно само
    // "спряталось", а осиротевший `sing-box.exe` от прошлой сессии,
    // который так и не был убит (например, приложение закрыли крестиком, а
    // не кнопкой "Отключить"). Он продолжает висеть в фоне: держит открытым
    // TUN-адаптер и порт Clash API, из-за чего установщик/деинсталлятор не
    // может полностью заменить/удалить файлы (они заняты процессом), а
    // если в это время запустить приложение заново — его собственное
    // состояние всё равно стартует как "disconnected" (см. ниже), но
    // реальный старый процесс продолжает жить независимо от него. Поэтому
    // на каждом старте принудительно убиваем все ЛИШНИЕ sing-box.exe —
    // гарантированно чистый старт, независимо от того, как закончилась
    // предыдущая сессия.
  ...
  Future<void> connect(SessionOptions options) async {
    ...
    try {
      ...
      // [НОВОЕ — реальная причина "Подключено, но заблокированные без VPN
      // сайты всё равно не открываются", подтверждено фото] Проверка выше
      // (`_verifyTunnelPassesTraffic`) идёт ЧЕРЕЗ локальный прокси-порт —
      // то есть напрямую в outbound sing-box'а, В ОБХОД TUN-адаптера. Она
      // честно доказывает, что VLESS/Reality-сервер живой и до него можно
      // достучаться, — но НИЧЕГО не говорит о том, поднялся ли сам
      // TUN-адаптер `VPNonLine`, через который идёт обычный, системный
      // трафик компьютера (браузер и т.д.). Ровно это и объясняет случаи,
      // когда предыдущая проверка проходила (сервер отвечал), а на деле
      // весь остальной интернет компьютера всё равно шёл в обход туннеля.
      // Здесь честно спрашиваем у самой Windows (через `Get-NetAdapter`),
      // существует ли сетевой адаптер `VPNonLine` и находится ли он в
      // состоянии "Up" — это прямое доказательство (или опровержение) того,
      // что TUN реально поднялся, а не просто предположение по косвенным
      // признакам.
  ...
  /// [НОВОЕ] Настоящая проверка через саму Windows, поднялся ли TUN-адаптер
  /// `VPNonLine` и в каком он состоянии — см. докстринг в connect() выше.
  /// `Get-NetAdapter` есть в PowerShell "из коробки" на любой современной
  /// Windows, дополнительных зависимостей не требует. До трёх коротких
  /// попыток с паузой — адаптеру иногда нужна секунда-другая, чтобы
  /// перейти из "Disconnected" в "Up" сразу после создания.
