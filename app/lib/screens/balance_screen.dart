...
/// Вкладка «Баланс».
///
/// Показывает сумму на счету, число ключей (активных из общего), состояние
/// подписки с кнопкой продления, предупреждение за 3 дня до конца и
/// реферальный блок. Пополнение ведёт на TopUpScreen, покупка нового
/// ключа — на PlansScreen.
///
/// Всё это строится на тех же двух запросах, что нужны для самой суммы:
/// `GET /user/profile` (balance, email, referral_link, referral_count,
/// referral_balance_all) и `GET /user/keys` (key_id, expiry_date,
/// devices_limit) — новых обращений к серверу экран не добавляет, чтобы не
/// утяжелять холодный старт (все вкладки монтируются сразу, см. `_visited`
/// в main.dart).
///
/// Данные перечитываются при возврате в приложение: пополнение уходит во
/// внешний браузер, а вкладка остаётся смонтированной в IndexedStack и
/// иначе показывала бы старую сумму.
...
class _BalanceScreenState extends State<BalanceScreen>
  ...
  DateTime? _loadedAt;
  // true, если на экране данные из локального кэша, а не свежий ответ
  // сервера. См. _restoreFromCache().
  ...
  /// Офлайн-копию профиля и ключей ведёт сам ApiClient — он же дедуплицирует
  /// getProfile()/getKeys(), которые этот экран запрашивает одновременно с
  /// MenuScreen и ConnectScreen при холодном старте. Экрану остаётся только
  /// показать, свежие данные или сохранённые.
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        // Битая загрузка не должна молча оставлять старые данные висеть без
        // объяснения. Если сохранённые данные на экране уже есть — это не авария,
        // а предупреждение: мягкий текст вместо сырой сетевой ошибки.
...
///
/// Полоса остатка считается от 30 дней, а не от реальной длительности
/// тарифа: `/user/keys` отдаёт только `expiry_date`, даты покупки там нет.
/// 30 дней здесь — шкала, а не заявление о длительности: точное число
/// оставшихся дней всегда подписано рядом цифрой.
