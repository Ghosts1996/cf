...
/// пакетов читается и передаётся сразу в два слоя: в tun-конфиг sing-box
/// (`exclude_package`/`include_package`) и в нативный Android VpnService
/// (`addDisallowedApplication`/`addAllowedApplication`). Второй слой обязателен:
/// именно он исключает UID выбранного приложения из VPN-сети Android,
/// чтобы оно видело обычное сетевое подключение без VPN. Чтобы применить
/// изменение к уже запущенному туннелю,
