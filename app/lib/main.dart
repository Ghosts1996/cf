...
class _RootShellState extends State<RootShell> {
  ...
  void initState() {
    super.initState();
    // [НОВОЕ — по просьбе: "при первом запуске пусть подгружает все данные
    // с меню страниц, чтобы при переходе между вкладками не висела
    // картинка как на скрине"] `_visited` (см. докстринг поля выше) на
    // старте специально содержит только вкладку "Главная" — это защита от
    // залпа из 5 одновременных сетевых запросов при холодном старте на
    // плохом мобильном интернете (см. подробности выше). Но раз речь
    // сейчас именно о том, чтобы данные ВСЕХ вкладок (Ключи/Баланс/
    // Серверы/Меню) были уже готовы к моменту, когда пользователь впервые
    // переключится на них, — а не только у "Главной", — достраиваем
    // остальные 4 вкладки сразу следом, но НЕ синхронно в этом же кадре:
    // `addPostFrameCallback` откладывает это на кадр ПОСЛЕ того, как
    // "Главная" уже отрисовалась и начала свою собственную загрузку, а не
    // добавляет ещё 4 конкурирующих сетевых запроса поверх неё в тот же
    // самый момент. Каждая вкладка при первом появлении в дереве (см.
    // `IndexedStack` ниже) сама вызывает свой initState() и стартует
    // собственную загрузку данных (getKeys()/getProfile()/getHosts() —
    // как и раньше, ничего в самих экранах не менялось) — просто теперь
    // это происходит один раз сразу при запуске приложения, а не при
    // первом нажатии на вкладку. У каждого запроса уже есть собственный
    // таймаут (см. api_client.dart::_requestTimeout) — конкурирующие
    // запросы больше не может "подвесить" экран навсегда, в худшем случае
    // какая-то вкладка покажет ошибку с кнопкой "Повторить" вместо вечного
    // спиннера.
